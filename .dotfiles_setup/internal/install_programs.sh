...
TASK_FAILURES=""
if [ "$DEBUG" -eq 1 ]; then
  SUMMARY_EVENT_FILE="/dev/null"
  SUMMARY_WARNING_FILE="/dev/null"
else
...
source "$MODULE_DIR/shell.sh"
source "$MODULE_DIR/neovim.sh"
source "$MODULE_DIR/neovim_tools.sh"
source "$MODULE_DIR/tmux.sh"
source "$MODULE_DIR/ssh.sh"

if [ "$DRY_RUN" -eq 1 ]; then
  info "installer start (debug simulation)"
else
  info "installer start"
fi

enter_module "profile/$PROFILE"
run_task "install_all_programs" "profile/$PROFILE"

if [ "$SUMMARY_FAILED_TASKS" -eq 0 ]; then
  while IFS= read -r fn; do
    run_task "$fn" "profile/$PROFILE"
  done < <(list_profile_tasks "$PROFILE")
else
  warn "required package installation failed; skipping dependent setup tasks"
fi

leave_module "profile/$PROFILE"

printf '\n'
done_log "installer complete"
if [ "$DEBUG" -eq 1 ]; then
  done_log "debug simulation completed without persistent changes"
else
  summary_print
...
if [ "$SUMMARY_FAILED_TASKS" -gt 0 ]; then
  exit 1
fi

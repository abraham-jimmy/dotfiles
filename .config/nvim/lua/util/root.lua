...
function M.get_root(bufnr)
  ...
  path = path and (uv.fs_realpath(path) or path) or nil
  ...
  if path then
    for _, client in pairs(vim.lsp.get_clients({ bufnr = bufnr })) do
      ...
      for _, client_path in ipairs(paths) do
        local root_path = uv.fs_realpath(client_path) or vim.fs.normalize(client_path)
        root_path = root_path ~= "/" and root_path:gsub("/+$", "") or root_path
        if path == root_path or root_path == "/" or vim.startswith(path, root_path .. "/") then
  ...
  if not root then
    ...
    local search_path = path and stat and stat.type == "directory" and path or path and vim.fs.dirname(path) or uv.cwd()

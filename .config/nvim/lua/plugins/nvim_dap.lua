vim.pack.add({ { src = "https://github.com/mfussenegger/nvim-dap.git" } }, { confirm = false, load = true })
...
local function load_launch_json()
  ...
  if vim.fn.filereadable(path) == 0 then
    vim.notify("launch.json not found at " .. path, vim.log.levels.WARN, { title = "nvim" })
    return
  end

  require("dap.ext.vscode").load_launchjs(path, { codelldb = { "c", "cpp", "cc" } })
  vim.notify("Loaded launch.json from " .. path, vim.log.levels.INFO, { title = "nvim" })
end
...
function M.setup()
  ...
  if not dap_ok then
    vim.schedule(function()
      vim.notify("nvim-dap is unavailable", vim.log.levels.WARN, { title = "nvim" })
    end)
    return
  end

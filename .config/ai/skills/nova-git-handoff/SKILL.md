...
Mutating commands start clean. Documentation agents use the validated `nova_git` tool instead of arbitrary shell commands. Before any commit, inspect status/diff, propose a concise message, and ask before staging. Review the staged diff before committing and report hash/message.
...
Never reset, discard, unstage, amend, force-push, stash, or create a recovery branch without explicit approval.

...
- After the final task, run `validate-spec <feature-folder>` in a new isolated session. It only verifies the spec's `Done` conditions, tags checks needing fresh human confirmation as `[MANUAL_VERIFY]`, and renames the folder to `<feature-slug>-DONE/` when every task and condition passes.

...
- Bootstrap uses `.dotfiles_setup/ensure_shell_startup.sh` to put guarded source blocks at the top of private `.zshrc`, `.bashrc`, and `.bash_profile` files. It leaves `.zshenv` absent by default so `ZDOTDIR` cannot bypass the root Zsh shim.
